...
void send_specs(ROTOR *AX_rot,ROTOR *EY_rot)
{
  ...
  xprintf("SPEC: AX_POffset     =%d",AX_POffset);      delay(SERDEL);
  xprintf("SPEC: EY_POffset     =%d",EY_POffset);      delay(SERDEL);
  xprintf("SPEC: AX_REFPOS      =%d",AX_REFPOS);       delay(SERDEL);
  xprintf("SPEC: EY_REFPOS      =%d",EY_REFPOS);       delay(SERDEL);
  xprintf("SPEC: AX_STEPS_DEGR  =%d",AX_STEPS_DEGR);   delay(SERDEL);
  xprintf("SPEC: EY_STEPS_DEGR  =%d",EY_STEPS_DEGR);   delay(SERDEL);
 #if MOTORTYPE == MOT_STEPPER
  xprintf("SPEC: AX_MotorSpeed  =%d",AX_MotorSpeed);   delay(SERDEL);
  xprintf("SPEC: AX_MotorAccel  =%d",AX_MotorAccel);   delay(SERDEL);
  xprintf("SPEC: EY_MotorSpeed  =%d",EY_MotorSpeed);   delay(SERDEL);
  xprintf("SPEC: EY_MotorAccel  =%d",EY_MotorAccel);   delay(SERDEL);
 #else
  xprintf("SPEC: MINSPEED       =%d",MINSPEED);        delay(SERDEL);
  xprintf("SPEC: MAXSPEED       =%d",MAXSPEED);        delay(SERDEL);
  xprintf("SPEC: L_DEGR_MAXSPEED=%d",L_DEGR_MAXSPEED); delay(SERDEL);
  xprintf("SPEC: H_DEGR_MINSPEED=%d",H_DEGR_MINSPEED); delay(SERDEL);
  xprintf("SPEC: D_DEGR_STOP    =%d",D_DEGR_STOP);     delay(SERDEL);
  xprintf("SPEC: PWMFreq        =%d",PWMFreq);         delay(SERDEL);
  xprintf("SPEC: MAX_PWM        =%d",MAX_PWM);         delay(SERDEL);
 #endif
}
